#-------------------------------------------------------------------------------
# Name:        srs_wb
# Purpose:     generate WB srs bcem fdd cfg
#
# Author:      jegong
#
# Created:     15/03/2017
    ...
    for tcSheet in dictTestcaseSheet:
        ...
        for testCase in testcaseDict:
            ...
            strCfgUl += strNoTab + '/*------- schedule LRX Section ----*/' + \
                       strNoTab + \
                       strNoTab + 'Column,LrxConfigMsg,AbsoluteTick,DeltaTick, Repeat,Num_SRS,Num_CELLS' + \
                       strNoTab

            strCfgUl += strNoTab + 'LrxConfigMsg,                -4,      16,     500,      1,        1'
            ...
            strCfgUl += strNoTab
            strCfgUl += \
                strNoTab + 'Column,LrxSrsConfigMsg,AbsoluteTick,DeltaTick, Repeat,Index,CellIdx,GrpIndex,StartPRB,NumofPRBs,NumOfPrbIdft,NumOfUsers,RepFactor,CombSelect,SrsAgcMode,AllocParity,GrpHopping,NcellID'
            for ue in range(num_ue):
                ...
                strCfgUl += strNoTab + 'LrxSrsConfigMsg,                 ' +str(toffset*16 - 4) + ',     ' + str(16*tsrs) + ',     500,   ' + str(ue) + ',      0,       0,      ' + str(startprb)+ ',        ' + str(numprb) + ',          ' + str(idftnrb) + ',           ' + str(num_ue) +',        1,       ' + \
                    str(ktc) + ',          4,         ' + str(allocparity) + ',         '  + str(grphop) + ',         ' + str(nCellId)

            strCfgUl += \
                strNoTab + 'Column,LrxSrsUserConfigMsg,AbsoluteTick,DeltaTick, Repeat,Index,Epsilon,UserId,CyclicShift'
            for u in range(num_ue):
                ...
                strCfgUl += \
                    strNoTab + 'LrxSrsUserConfigMsg,                 ' + str(toffset*16 - 4) + ',       ' + str(16*tsrs) + ',    500,    ' + str(u) + ',      0,     ' + str(u) + ',          ' + str(cycicshift)
